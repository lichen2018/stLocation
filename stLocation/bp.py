...
def cell_marginals(
    ...
    neighborhood = neighborhood > 0 if neighborhood is not None else circle(3).astype(bool)
    ...
    bp.run(precision=precision, max_iter=max_iter, n_threads=20)
    return bp.marginals()
...
def run_bp(
    ...
    """
    neighborhood = np.ones((k, k)) if square else circle(k)
