...
class NegativeBinomialMixture(PyroModule):
    def __init__(
        ...
        if not ((w is None) == (mu is None) and (w is None) == (var is None)):
            print("All or none of `w`, `mu`, `var` must be provided.")
        if (w is not None) and (n != len(w) or n != len(mu) or n != len(var)):
            print(f"`w`, `mu`, `var` must have length {n}.")
...
def conditionals(
    ...
    if "counts" not in vi_results:
        if bins is None:
            print("`vi_results` indicate binning was used, but `bins` was not provided")
        ...
        for label in np.unique(bins):
            if label > 0:
                ...
                if set(_params.keys()) != {"w", "mu", "var"}:
                    print("`params` must contain exactly the keys `w`, `mu`, `var`.")
    else:
        samples[0] = X.flatten()
